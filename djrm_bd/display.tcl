...
#create_project -force -part xc7z010clg400-2 -name system
#create_project -force -part xc7z010clg400-2 -in_memory
...
# 67Hz gives near to 40MHz pixel clock and good freq. synth. with 125 MHz input
create_bd_cell -type ip -vlnv clifford.at:ip:simplehdmi framebuffer
set_property -dict "
	CONFIG.SVO_MODE    800x600R
	CONFIG.SVO_FRAMERATE      67
...
set_property -dict "
	...
	CONFIG.CLKOUT3_REQUESTED_OUT_FREQ [expr "double([get_property CONFIG.FREQ_HZ [get_bd_pins framebuffer/clk_7x_pixel]])/1e6"]
...
create_bd_port -dir O openldi_clk_p
create_bd_port -dir O -from 2 -to 0 openldi_a_n
create_bd_port -dir O -from 2 -to 0 openldi_a_p
...
apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config {Master "/framebuffer/mem" Clk "Auto"}  [get_bd_intf_pins processing_system7_0/S_AXI_HP0]
#apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config {Master "/ps7/M_AXI_GP0" Clk "Auto"}  [get_bd_intf_pins framebuffer/cfg]
apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config {Master "/processing_system7_0/M_AXI_GP0" Clk "Auto"}  [get_bd_intf_pins framebuffer/cfg]

#connect_bd_net [get_bd_ports clk] [get_bd_pins clk_wiz/clk_in1]
set_property -dict [list CONFIG.OVERRIDE_MMCM {true} CONFIG.MMCM_COMPENSATION {INTERNAL}] [get_bd_cells clk_wiz]

connect_bd_net [get_bd_pins processing_system7_0/FCLK_CLK0] [get_bd_pins clk_wiz/clk_in1]
connect_bd_net [get_bd_pins framebuffer/resetn] [get_bd_pins processing_system7_0/FCLK_RESET0_N]
...
connect_bd_net [get_bd_pins framebuffer/clk_7x_pixel] [get_bd_pins clk_wiz/clk_out3]
...
foreach it {openldi_clk_n openldi_clk_p openldi_a_n openldi_a_p} {
	connect_bd_net [get_bd_ports $it] [get_bd_pins framebuffer/$it]
}

set_property -dict {offset 0x00000000 range 256M} [get_bd_addr_segs {framebuffer/mem/SEG_processing_system7_0_HP0_DDR_LOWOCM}]
set_property -dict {offset 0x43000000 range  64k} [get_bd_addr_segs {processing_system7_0/Data/SEG_framebuffer_Reg}]
